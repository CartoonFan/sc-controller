...
class ActionParser(object):
    ...
    def from_json_data(self, data, key=None):
        ...
        if decoders:
            for cls in sorted(decoders, key=lambda a: a.PROFILE_KEY_PRIORITY):
                # Profile version is not yet used anywhere
    ...
    def restart(self, string):
        ...
        try:
            self.tokens = [
                ...
                for (type, string, trash, trash,
                     trash) in generate_tokens(iter([string]).__next__)
    ...
    def _parse_parameter(self):
        ...
        if t.type == TokenType.NAME:
            ...
            elif (self._tokens_left() and t.value in Action.ALL
    ...
    def _parse_parameters(self):
        ...
        if t.type != TokenType.OP or t.value != "(":
            raise ParseError("Expected '(' of parameter list, got '%s'" %
                             (t.value, ))
    ...
    def _create_action(self, cls, *pars):
        try:
            ...
            print >> sys.stderr, e
            raise ParseError("Invalid number of parameters for '%s'" %
                             (cls.COMMAND))
