...
# Must match number of axis fields in HIDControllerInput and values in AxisType
AXIS_COUNT = 17
# Must match (or be less than) number of bits in HIDControllerInput.buttons
BUTTON_COUNT = 32
...
class HIDController(USBDevice, Controller):
    ...
    def _build_axis_maping(self, axis, config, mode=AxisMode.AXIS):
        ...
        if axis_config:
            ...
            button = 0
            if AxisType(target) in (
                    AxisType.AXIS_LPAD_X,
                ...
                if x[1] == ItemType.Constant:
                    ...
                    if kind in AXES:
                        ...
                        if next_axis + 1 < AXIS_COUNT:
                            log.debug(
                                "Found hat #%s at bit %s", int(next_axis), total)
                            if config:
                                target, axis_data = self._build_axis_maping(
                        ...
                        log.debug(
                            "Skipped over %s bits for %s at bit %s",
                            count * size,
                            kind,
                            total,
                        )
