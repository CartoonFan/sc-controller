...
class TestActions(object):
    ...
    def test_axis(self):
        ...
        """
        assert isinstance(parser.from_json_data(
            {'action': 'axis(ABS_X)'}), AxisAction)
        assert parser.from_json_data(
            {'action': 'axis(ABS_X)'}).id == Axes.ABS_X
    ...
    def test_raxis(self):
        ...
        """
        assert isinstance(parser.from_json_data(
            {'action': 'raxis(ABS_X)'}), RAxisAction)
        assert parser.from_json_data(
            {'action': 'raxis(ABS_X)'}).id == Axes.ABS_X
    ...
    def test_hats(self):
        ...
        """
        assert isinstance(parser.from_json_data(
            {'action': 'hatup(ABS_X)'}), HatUpAction)
        assert isinstance(parser.from_json_data(
            {'action': 'hatdown(ABS_X)'}), HatDownAction)
        assert isinstance(parser.from_json_data(
            {'action': 'hatleft(ABS_X)'}), HatLeftAction)
        assert isinstance(parser.from_json_data(
            {'action': 'hatright(ABS_X)'}), HatRightAction)

        assert parser.from_json_data(
            {'action': 'hatup(ABS_X)'}).id == Axes.ABS_X
        assert parser.from_json_data(
        ...
        assert parser.from_json_data(
            {'action': 'hatright(ABS_X)'}).max == STICK_PAD_MAX - 1
    ...
    def test_mouse(self):
        ...
        assert parser.from_json_data(
            {'action': 'trackpad()'})._mouse_axis is None
        assert parser.from_json_data(
            {'action': 'mouse(REL_WHEEL)'})._mouse_axis == Rels.REL_WHEEL
    ...
    def test_mouseabs(self):
        ...
        """
        assert parser.from_json_data(
            {'action': 'mouseabs(REL_X)'})._mouse_axis == Rels.REL_X
        assert parser.from_json_data(
            {'action': 'mouseabs()'})._mouse_axis is None
    ...
    def test_area(self):
        ...
        assert parser.from_json_data(
            {'action': 'area(10, 10, 50, 50)'}).coords == (10, 10, 50, 50)
    ...
    def test_relarea(self):
        ...
        assert parser.from_json_data(
            {'action': 'relarea(10, 10, 50, 50)'}).coords == (10, 10, 50, 50)
    ...
    def test_winarea(self):
        ...
        assert parser.from_json_data(
            {'action': 'winarea(10, 10, 50, 50)'}).coords == (10, 10, 50, 50)
    ...
    def test_relwinarea(self):
        ...
        """
        assert isinstance(parser.from_json_data(
            {'action': 'relwinarea(10, 10, 50, 50)'}), RelWinAreaAction)
        assert parser.from_json_data(
            {'action': 'relwinarea(10, 10, 50, 50)'}).coords == (10, 10, 50, 50)
    ...
    def test_gyro(self):
        ...
        assert parser.from_json_data(
            {'action': 'gyro(ABS_X, ABS_Y)'}).axes[1] == Axes.ABS_Y
        assert parser.from_json_data(
            {'action': 'gyro(ABS_X, ABS_Y)'}).axes[2] is None
        assert parser.from_json_data(
    ...
    def test_gyroabs(self):
        ...
        """
        assert isinstance(parser.from_json_data(
            {'action': 'gyroabs(ABS_X)'}), GyroAbsAction)

        assert parser.from_json_data(
            {'action': 'gyroabs(ABS_X)'}).axes[0] == Axes.ABS_X
        assert parser.from_json_data(
            {'action': 'gyroabs(ABS_X)'}).axes[1] is None
        assert parser.from_json_data(
            {'action': 'gyroabs(ABS_X, ABS_Y)'}).axes[1] == Axes.ABS_Y
        assert parser.from_json_data(
            {'action': 'gyroabs(ABS_X, ABS_Y)'}).axes[2] is None
        assert parser.from_json_data(
    ...
    def test_resetgyro(self):
        ...
        """
        assert isinstance(parser.from_json_data(
            {'action': 'resetgyro()'}), ResetGyroAction)
    ...
    def test_button(self):
        ...
        """
        assert isinstance(parser.from_json_data(
            {'action': 'button(KEY_X)'}), ButtonAction)

        assert parser.from_json_data(
            {'action': 'button(KEY_X)'}).button == Keys.KEY_X
        assert parser.from_json_data(
            {'action': 'button(KEY_X)'}).button2 is None
        assert parser.from_json_data(
            {'action': 'button(KEY_X, KEY_Z)'}).button == Keys.KEY_X
        assert parser.from_json_data(
            {'action': 'button(KEY_X, KEY_Z)'}).button2 == Keys.KEY_Z
