...
class TestActions(object):
    ...
    def test_hats(self):
        ...
        assert (
            parser.from_json_data({"action": "hatup(ABS_X)"}).max == STICK_PAD_MIN + 1
        )
        assert (
            parser.from_json_data({"action": "hatdown(ABS_X)"}).max == STICK_PAD_MAX - 1
        )
        assert (
            parser.from_json_data({"action": "hatleft(ABS_X)"}).max == STICK_PAD_MIN + 1
        )
        assert (
    ...
    def test_area(self):
        ...
        assert parser.from_json_data({"action": "area(10, 10, 50, 50)"}).coords == (
            10,
            10,
            50,
            50,
    ...
    def test_relarea(self):
        ...
        assert parser.from_json_data({"action": "relarea(10, 10, 50, 50)"}).coords == (
            10,
            10,
            50,
            50,
    ...
    def test_winarea(self):
        ...
        assert parser.from_json_data({"action": "winarea(10, 10, 50, 50)"}).coords == (
            10,
            10,
            50,
            50,
    ...
    def test_gyroabs(self):
        ...
        assert (
            parser.from_json_data({"action": "gyroabs(ABS_X, ABS_Y)"}).axes[2] is None
        )
        assert (
