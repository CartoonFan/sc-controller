...
class TestModifiers(object):
    ...
    def test_circular(self):
        ...
        """
        if not isinstance(
                _parse_compressed("circular(axis(ABS_X))"),
                CircularModifier):
            raise AssertionError
        if not isinstance(
    ...
    def test_circularabs(self):
        ...
        if not isinstance(
            ...
            raise AssertionError
        if not isinstance(
                _parse_compressed("circularabs(axis(REL_WHEEL))"),
                CircularAbsModifier):
            raise AssertionError
    ...
    def test_sens(self):
        ...
        if _parse_compressed(
                "sens(2, axis(ABS_X))").strip().get_speed() != (2.0,):
            raise AssertionError
        if _parse_compressed(
                "sens(2, 3, mouse())").strip().get_speed() != (2.0, 3.0):
            raise AssertionError
        if _parse_compressed(
        ...
        if not (
            isinstance(
                a.action,
                MouseAction) and a.action.get_speed() == (
                2.0,
                3.0)):
            raise AssertionError
