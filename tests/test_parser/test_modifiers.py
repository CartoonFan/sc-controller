...
class TestModifiers(object):
    ...
    def test_circular(self):
        ...
        """
        if not isinstance(_parse_compressed("circular(axis(ABS_X))"),
                          CircularModifier):
            raise AssertionError
        if not isinstance(_parse_compressed("circular(axis(REL_WHEEL))"),
                          CircularModifier):
            raise AssertionError
    ...
    def test_circularabs(self):
        ...
        """
        if not isinstance(_parse_compressed("circularabs(axis(ABS_X))"),
                          CircularAbsModifier):
            raise AssertionError
        if not isinstance(_parse_compressed("circularabs(axis(REL_WHEEL))"),
                          CircularAbsModifier):
            raise AssertionError
    ...
    def test_doubleclick(self):
        ...
        if not (isinstance(a.normalaction, AxisAction)
                and a.normalaction.id == Axes.ABS_Y):
            raise AssertionError
        if a.holdaction:
            raise AssertionError
        # With all parameters
    ...
    def test_hold_doubleclick_combinations(self):
        ...
        if not (isinstance(a.action, AxisAction)
            ...
            raise AssertionError
        if not (isinstance(a.holdaction, AxisAction)
                and a.holdaction.id == Axes.ABS_RX):
            raise AssertionError
        if not (isinstance(a.normalaction, AxisAction)
                and a.normalaction.id == Axes.ABS_Z):
            raise AssertionError
        ...
        if not (isinstance(a.action, AxisAction)
                and a.action.id == Axes.ABS_Z):
            raise AssertionError
        if not (isinstance(a.holdaction, AxisAction)
                and a.holdaction.id == Axes.ABS_RZ):
            raise AssertionError
        if not (isinstance(a.normalaction, AxisAction)
                and a.normalaction.id == Axes.ABS_X):
            raise AssertionError
    ...
    def test_sens(self):
        ...
        """
        # Simple stuff
        if _parse_compressed("sens(2, axis(ABS_X))").strip().get_speed() != (
                2.0, ):
            raise AssertionError
        if _parse_compressed("sens(2, 3, mouse())").strip().get_speed() != (
                2.0, 3.0):
            raise AssertionError
        if _parse_compressed("sens(2, 3, 4, gyro(ABS_RZ, ABS_RX, ABS_Z))"
        ...
        if not (isinstance(a.action, MouseAction)
                and a.action.get_speed() == (2.0, 3.0)):
            raise AssertionError
        ...
        if not (isinstance(a.action, MouseAction)
                and a.action.get_speed() == (1.0, 1.0)):
            raise AssertionError
